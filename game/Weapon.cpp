...
/*
================
rvWeapon::AttackHyperblaster
================
*/
void rvWeapon::AttackHyperblaster(bool altAttack, int num_attacks, float spread, float fuseOffset, float power) {
	idVec3 muzzleOrigin;
	idMat3 muzzleAxis;
	...
	if (!gameLocal.isClient) {
		...
		if (altAttack ? wfl.attackAltHitscan : wfl.attackHitscan) {
			...
			LaunchProjectilesMod(dict, muzzleOrigin, muzzleAxis, num_attacks, spread, fuseOffset, power);
		}
		//asalmon:  changed to keep stats even in single player 
		statManager->WeaponFired(owner, weaponIndex, num_attacks);
...
/*
================
rvWeapon::LaunchProjectilesHyperblasterMod
================
*/
void rvWeapon::LaunchProjectilesMod(idDict& dict, const idVec3& muzzleOrigin, const idMat3& muzzleAxis, int num_projectiles, float spread, float fuseOffset, float power) {
	idProjectile*	proj;
	idEntity*		ent;
	int				i;
	float			spreadRad;
